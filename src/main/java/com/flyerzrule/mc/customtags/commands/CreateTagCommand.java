...
public class CreateTagCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        if (cmd.getName().equalsIgnoreCase("tagcreate")) {
            if (args.length < 6) {
                return false;
            }
            ...
            String tagId = parsedArgs.get(0).toLowerCase();
            ...
            ReturnCode result = db.createTag(newTag, TagUpdateMethod.COMMAND, sender.getName());

            if (result == ReturnCode.SUCCESS) {
                sender.sendMessage(
                        Component.text().content("Tag " + tagId + " created!").color(NamedTextColor.GREEN).build());
                return true;
            } else if (result == ReturnCode.ALREADY_EXISTS) {
                sender.sendMessage(Component.text().content("Tag " + tagId + " already exists!")
                        .color(NamedTextColor.RED).build());
                return false;
            } else {
                sender.sendMessage(Component.text().content("Error creating tag " + tagId + "!")
                        .color(NamedTextColor.RED).build());
                return false;
            }
        }
